...
type AlbumPhotoSliderProps = {}

export const AlbumSlider: React.FC<AlbumPhotoSliderProps> = ({}) => {
        ...
        max-tb:flex-col max-tb:p-0">
            ...
            max-ph:py-6 max-ph:gap-y-6">
                ...
                max-ph:gap-y-6">
                    ...
                    max-ph:gap-x-5 max-ph:justify-center">
                        <Button size={ESizes.x} type={EType.border}
                                onClick={() => {
                                    addPurchase(activeId as string,
                                        activeMenu,
                                        photos.find(photo => photo.id === activeId)!.file)
                                    setIsActive(false)
                        }}>
                            {content.addCart}
                        </Button>
                        <Button size={ESizes.x} type={EType.blue} styles={'rounded-full !px-6'} onClick={() => {
                            navigate('/cart')
                            setIsActive(false)
                        }}>
                            {content.makeFavor}
